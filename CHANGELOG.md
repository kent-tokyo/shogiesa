...
### Added
- `filter`/`balance`/`sample`/`pack --manifest PATH` writes an opt-in run manifest (JSON): shogiesa version, git sha, schema/pack-format version, full command args, input path + a non-cryptographic content hash (change-detection only, not a verifiable checksum), records read/kept/dropped, drop-reason counts, labeled/unlabeled record counts, MultiPV-candidate coverage, score-bound distribution, and (for `filter`) the resolved `QualityConfig`. `label` and `split` are not covered — `split` already has its own tailored `manifest.json`, `label` has different per-worker/per-engine fields that don't fit this shape.
