...
その深さをスキップします — 強制終了後に大規模ラベリングを安価に再開するのに便利ですが、
出力ファイルに実際に書き込まれているものからしか作業リストを再構築できません。
`--preserve-order` 実行中に、強制終了前に自身のメモリ上バッファから失われた作業までは
回復できません。`--replace-existing` は同じ深さの既存観測を重複追加ではなく上書きします
（意図的な再ラベル用）。
...
`preserve_order`、そして(`--cache-dir` 使用時は)cache hit/miss件数、`cache_hit_rate`、
