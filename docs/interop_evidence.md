...
| KIF -> JSONL | `sample.kif`, `sample_handicap.kif`, `variation.kif` | mainline, handicap, and supported variation provenance | nested variations |
...
| JSONL -> pack -> JSONL | `shogiesa-pack` tests and CLI schema fixtures v1-v11 | current format 11 round-trips through JSONL | editing pack as a primary format |
